...
<head>
    ...
    <script type="text/javascript">
		...
		if(n) $("#pageNo").val(n);
	    if(s) $("#pageSize").val(s);
	          $("#searchForm").attr("action","${ctxsys}/trace/chooseProducts");
	          $("#searchForm").submit();
	    	return false;
	    }
	    function eblabelLs(){
...
<body>
	...
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="merchandise:pro:view"><li class="active"><a href="${ctxsys}/trace/chooseProducts">商品列表</a></li></shiro:hasPermission>
	</ul>
	...
	<table  class="table table-striped table-condensed table-bordered" >
		...
		<c:forEach items="${page.list}" var="productlist" varStatus="status">
			<tr>
			   <td class="center123"><input type="radio" name="ktvs" class="kty chooseItem"  value="${productlist.productId}" code="${productlist.tracingSourceCode}"></td>
			  <td class="center123">${productlist.productId}</td>
				<td class="center123 productName" title=" ${productlist.productName}">
 ...
 <script type="text/javascript">
       ...
       $('body').on('click','.chooseItem',function(){
    	   ...
    	   var prodctName=$(this).parents("tr").find(".productName").text().trim();
    		   $("#chooseIds").val(id);
