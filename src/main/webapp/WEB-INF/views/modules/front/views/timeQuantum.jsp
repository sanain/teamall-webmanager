...
<body>
		<div class="jztime">
					...
					<input type="hidden" id="numSourceTime" name="numSourceTime" value="${numSourceTime}"/>
					<h1>请选择就诊时间</h1>
...
$(function(){ 
		...
		for(var i=0; i < obj.length; i++){
		  var html = [];  
				 html.push('<option value="'+obj[i].timestypeNoName+'R'+obj[i].timestypeNo+'">'+obj[i].timestypeNoName+'</option>'); 
...
$("#gh_submit").click(function(){  
  var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
    parent.window.location.href = "${ctx}/userinfo/recordconfirm?sourceid="+$("#sourceid").val()+"&numSourceTime="+$("#numSourceTime").val()+"&timestypes="+$("#sel").val();
  	 parent.layer.close(index);
});	
