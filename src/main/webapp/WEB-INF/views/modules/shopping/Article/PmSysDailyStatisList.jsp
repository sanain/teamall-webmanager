...
function page(n,s){
	if(n) $("#pageNo").val(n);
	if(s) $("#pageSize").val(s);
	$("#searchForm").attr("action","${ctxsys}/PmSysDailyStatis/list");
	$("#searchForm").submit();
	return false;
}
</script>     
...
<body>
	...
	<form:form id="searchForm" modelAttribute="pmSysDailyStatis" action="${ctxsys}/PmSysDailyStatis/list" method="post" class="breadcrumb form-search ">
    	...
    	<div class="check-box">
	        <p>导出选项<img class="check-del1" src="${ctxStatic}/sbShop/images/xxx-rzt.png" alt=""></p>
	          <ul class="mn1">
		          ...
		          <li class="checkbox"><input type="checkbox" class="kl" name="syllable" value="2"><label><i></i>当天产生总积分数</label></li>
		          <li class="checkbox"><input type="checkbox" class="kl" name="syllable" value="3"><label><i></i>折扣金额</label></li>
		          <li class="checkbox"><input type="checkbox" class="kl" name="syllable" value="4"><label><i></i>今日新增资金池金额</label></li>
		          <li class="checkbox"><input type="checkbox" class="kl" name="syllable" value="5"><label><i></i>资金池余额</label></li>
              </ul>
            <div class="check-btn">
		...
		<c:forEach items="${page.list}" var="pack" varStatus="i">
			<tr id="${pack.id}">
				...
				<td> <fmt:formatDate value="${pack.createTime}" /></td>
				 <c:set var="rewardLove" value="${pack.rewardLove+rewardLove}"></c:set>
				<td><fmt:formatNumber type="number" value="${pack.rewardLove}" pattern="0.00" maxFractionDigits="4"/></td>
				 <c:set var="orderAmt" value="${pack.shopPartAmt+orderAmt}"></c:set>
                <td><fmt:formatNumber type="number" value="${pack.shopPartAmt}" pattern="0.00" maxFractionDigits="4"/></td>
                 <c:set var="dayPoolAmt" value="${pack.dayPoolAmt+dayPoolAmt}"></c:set>
                <td><fmt:formatNumber type="number" value="${pack.dayPoolAmt}" pattern="0.00" maxFractionDigits="4"/></td>
                <c:set var="poolAmt" value="${pack.poolAmt+poolAmt}"></c:set>
                <td><fmt:formatNumber type="number" value="${pack.poolAmt}" pattern="0.00" maxFractionDigits="4"/></td>
			</tr>
		</c:forEach>
		<tr>
			  <th>合计:</th>
			  <th></th>
              <th><fmt:formatNumber type="number" value="${rewardLove}" pattern="0.00" maxFractionDigits="4"/></th>
              <th><fmt:formatNumber type="number" value="${orderAmt }" pattern="0.00" maxFractionDigits="4"/></th>
              <th><fmt:formatNumber type="number" value="${dayPoolAmt}" pattern="0.00" maxFractionDigits="4"/></th>
              <th><fmt:formatNumber type="number" value="${poolAmt}" pattern="0.00" maxFractionDigits="4"/></th>
		</tr>
	</table>
	...
	$('body').on('click','.kl',function(){
     ...
     $('#fromNewActionSbM').click(function(){
	     $.ajax({
			    ...
			    url : "${ctxsys}/PmSysDailyStatis/exsel",   
