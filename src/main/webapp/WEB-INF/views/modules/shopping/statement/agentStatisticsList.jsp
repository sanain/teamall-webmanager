...
<head>
    ...
    <script type="text/javascript">
        ...
        function page(n,s){
            if(n) $("#pageNo").val(n);
            if(s) $("#pageSize").val(s);
            $("#searchForm").attr("action","${ctxsys}/agentStatement/statisticsList");
            $("#searchForm").submit();
            return false;
        }
    </script>
...
<form:form id="searchForm" modelAttribute="pmBusinessStatistics" action="${ctxsys}/agentStatement/handoverList" method="post" class="breadcrumb form-search ">
        ...
        <li><label>快捷选择:</label>
            <select name="quickTime" onchange="clearTimeRange()">
                ...
                <option value="2" <c:if test="${quickTime == 2}">selected</c:if>>${type==1?"近七天":"近六月"}</option>
                    <option value="3" <c:if test="${quickTime == 3}">selected</c:if>>${type==1?"近三十天":"近六月"}</option>
            </select>
        </li>
        <li><label>显示方式:</label>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed" >
    <tr class="title-tr" style="text-align: center">
        <th>序号</th>
        <th  <c:if test="${isAll == 0}">colspan="2"</c:if>>名称</th>
...
<script type="text/javascript">
    ...
    $('#fromNewActionSbM').click(function(){
        $.ajax({
            ...
            url : "${ctxsys}/agentStatement/statisticsExcel",
