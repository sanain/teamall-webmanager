...
<div class="content">
    <div class="info-div">
        <div class="layui-inline">
            <form method="post" action="${ctxsys}/agentStatement/businessDaily">
                ...
                <div class="layui-input-inline"><label class="layui-form-label">当前代理商:</label>  <input type="text"  class="input-medium" <c:if test="${!isAgent}"> onclick="chooseAgent()"</c:if> readonly id="agentName" name="agentName" value="${agentName}"  style=" width: 200px;margin: 4px 0px;"></li>

                </div>
                <div class="layui-input-inline">
                    <label class="layui-form-label">显示方式：</label>
                    <input type="radio"  id="all" name="isAll" onclick="chooseShop(1)" value="1" style="margin: 15px 0px;" <c:if test="${isAll == 1}">checked="checked"</c:if>>汇总
                    <input type="radio"  id="byshop" onclick="chooseShop(0)" value="0" name="isAll" style="margin: 15px 0px;" <c:if test="${isAll == 0}">checked="checked"</c:if>>指定门店
                    </select>
                </div>

                <div class="layui-input-inline shop-name-div"
                     <c:if test="${shopName == null || ''.equals(shopName)}">
                        style="display: none"
                     </c:if>
                ><label class="layui-form-label">当前门店:</label>  <input type="text" onclick="chooseShop(0)" class="input-medium" readonly id="shopName" name="shopName" value="${shopName}"  style=" width: 200px;margin: 4px 0px;"></li>
                </div>
        ...
        <c:forEach items="${itemList}" var="item" varStatus="vs">
            ...
            <div class="info-div type-div">
                ...
                <div class="info-div ranking-info" style="text-align: center">
                    <h5>菜品销量排行（前10）</h5>
                    <c:if test="${rankingList == null || rankingList.size() == 0}">
                        暂无数据
                    </c:if>
                    <c:if test="${rankingList != null || rankingList.size() != 0}">
                        <ul class="head">
            ...
            layer.open({
                ...
                yes: function(index, layero){ //或者使用btn1
                    ...
                    if(content==""){
                       ...
                       $(".shop-name-div").css("display","inline-block");
                        layer.close(index);
                    }
