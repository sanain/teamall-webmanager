...
<body>
    <div class="c-context">
      <ul class="nav-ul" style="margin:0px">
          ...
          <li><a class="active" href="${ctxsys}/Order/saleorderAftersalelist?orderId=${ncMessageTable.orderId}">订单退货处理</a></li>
          ...
          <li><a href="${ctxsys}/Order/saleorderendreply?orderId=${ncMessageTable.orderId}">最终批复</a></li>
		   <li><a href="${ctxsys}/NcMessageTable/saleordernc?orderId=${ncMessageTable.orderId}">nc同步</a></li>
      </ul>
    
	</div>
	<input type="button" value="重新同步下单" onclick="To_resynchronize()" style="margin:10px"/>
...
function To_resynchronize(){
	var msg = "同步成功";
	$.ajax({
	 					url : "${ctxsys}/NcMessageTable/saleorder_after_nc_reset?orderId=${ncMessageTable.orderId}&saleId=${saleId}",
	 					type : 'post',
	 					data : {
	 					},
	 					cache : false,
	 					success : function(data) {
							console.log(data);
	 						...
	 						if(data.code=='00'){
								...
								window.location.href="${ctxsys}/NcMessageTable/saleorderAfternc?orderId=${ncMessageTable.orderId}&saleId=${saleId}"
	 						} else {
