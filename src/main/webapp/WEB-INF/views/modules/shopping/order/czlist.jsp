...
<head>
	...
	<script type="text/javascript">
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxsys}/Order/czlist");
			$("#searchForm").submit();
	    	return false;
	    }
...
<body>
	<ul class="nav nav-tabs">
		<li ><a href="${ctxsys}/Order">商品订单列表</a></li>
		<li><a href="${ctxsys}/Order/duolist">夺宝订单列表</a></li>
		<li class="active"><a href="${ctxsys}/Order/czlist">充值订单列表</a></li>
		<!-- 
		<shiro:hasPermission name="merchandise:order:edit"><li><a href="${ctxsys}/Order/form">订单添加</a></li></shiro:hasPermission>
	 -->
	</ul>
	 <form:form id="searchForm" modelAttribute="ebOrder" action="${ctxsys}/Order/czlist" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">
		    <li><form:input path="orderNo" htmlEscape="false" maxlength="50" class="input-medium"  placeholder="请输入订单编号"/></li>
			 <%-- <li><form:select path="type"  htmlEscape="false" maxlength="50"  class="input-medium">
                   ...
                   <form:option value="2">夺宝订单</form:option>
               </form:select>  
		        </li>  --%>
		...
		<c:forEach items="${page.list}" var="orderlist" varStatus="status">
			<tr>
			    <td>${status.index+1}</td>
			     <td>  <c:if test="${orderlist.type==1}">商品订单</c:if><c:if test="${orderlist.type==2}">夺宝订单</c:if><c:if test="${orderlist.type==3}">充值订单</c:if></td>
			   ...
			   <shiro:hasPermission name="merchandise:order:edit"><td>
					...
					<%-- <a href="${ctxsys}/Order/delete?id=${orderlist.orderId}" onclick="return confirmx('要删除该区域及所有子区域项吗？', this.href)">删除</a> --%>
