...
<head>
	...
	<script type="text/javascript">
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxsys}/Order/lists");
			$("#searchForm").submit();
	    	return false;
	    }
	    $(function(){
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxsys}/Order/lists">商品线上订单列表</a></li>
	</ul>
	 <form:form id="searchForm" modelAttribute="ebOrder" action="${ctxsys}/Order" method="post" class="breadcrumb form-search ">
		...
		<input name="onoffLineStatus" type="hidden" value="1"/>
		<ul class="ul-form">
		    <li>
		     <label>订单编号</label>
	...
	<table id="treeTable" class="table table-striped table-condensed table-bordered" >
		...
		<c:forEach items="${page.list}" var="orderlist" varStatus="status">
			<tr>
			    ...
			    <td><fmt:formatNumber type="number" value="${orderlist.orderAmount}" pattern="0.00" maxFractionDigits="2"/></td>
			     <c:set var="sRealAmount" value="${orderlist.sRealAmount+orderAmount}"></c:set> 
				<td><fmt:formatNumber type="number" value="${orderlist.sRealAmount}" pattern="0.00" maxFractionDigits="2"/></td>
				<td><fmt:formatNumber type="number" value="${orderlist.totalLove}" pattern="0.00" maxFractionDigits="2"/></td>
				<td>${fns:replaceMobile(orderlist.mobile)}</td>
				<td>         
				...
				<c:if test="${orderlist.status!=1&&orderlist.status!=null&&orderlist.status!=2&&orderlist.status!=3}">
			...
			<th><fmt:formatNumber type="number" value="${sRealAmount}" pattern="0.00" maxFractionDigits="2"/></th>
			<th></th>
			<th></th>
			<th></th>
			<th></th>
