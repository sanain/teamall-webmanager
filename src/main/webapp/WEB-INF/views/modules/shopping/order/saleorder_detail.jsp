...
<div class="order">
    ...
    <div class="order-state">
        <p>当前订单状态：<c:if test="${ebOrder.status==null}">等待买家付款</c:if> <c:if test="${ebOrder.status==1}">等待买家付款</c:if><c:if test="${ebOrder.status==2}">等待发货</c:if><c:if test="${ebOrder.status==3}">已发货,待收货</c:if><c:if test="${ebOrder.status==4}">交易成功，已完成</c:if><c:if test="${ebOrder.status==5}">已关闭</c:if></p>
    </div>
    <!--订单跟踪-->
    ...
    <div class="order-msg">
        ...
        <c:if test="${ebOrder.onoffLineStatus==1||ebOrder.onoffLineStatus==4}">
                ...
                <c:if test="${ebOrder.status==2 || ebOrder.status==3||ebOrder.status==4||ebOrder.status==7||ebOrder.status==9}">
                    <ul>
                        ...
                        <c:if test="${ebOrder.shippingMethod == 5}">
                            ...
                            <li <c:if test="${ebOrder.status != 2}">style="display: none"</c:if>><span>选择配送人员：</span><a class="btn btn-primary" style=" background-color: #393D49;" href="javascript:;" onclick="chooseDeliveryStaff();">选择</a></li>
                        </c:if>
            ...
            <%--<ul>--%>
            <%--<li>配送信息</li>--%>
            <%--&lt;%&ndash;<li><span>配送方式：</span><span>快递</span></li>&ndash;%&gt;--%>
            <%--<li><span>运&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;费：</span><span>¥${ebOrder.realFreight}</span></li>--%>
            <%--<li><span>备&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;注：</span><span>¥${ebOrder.postscript}</span></li>--%>
            <%--&lt;%&ndash;<li><span>承运公司：</span><span>${ebOrder.logisticsCompany }</span></li>&ndash;%&gt;--%>
            <%--&lt;%&ndash;<li>&ndash;%&gt;--%>
            <%--&lt;%&ndash;<span>运单编号：</span><span>${ebOrder.expressNumber }</span>&ndash;%&gt;--%>
            <%--&lt;%&ndash;<!-- class="look-a" --> <a  href="http://www.kuaidi.com/chaxun?com=${ebOrder.logisticsComCode}&nu=${ebOrder.expressNumber}">查看物流</a>&ndash;%&gt;--%>
            <%--&lt;%&ndash;</li>&ndash;%&gt;--%>
            <%--</ul>--%>
            <%--</c:if>--%>
        </c:if>
        <ul>
            ...
            <li><span>应付金额：</span><span>¥<c:if test="${ebOrder.onoffLineStatus==3||ebOrder.onoffLineStatus==2}">
                <fmt:formatNumber type="number" value="${(ebOrder.orderAmount*ebOrder.returnRatio/100)}" pattern="0.00" maxFractionDigits="2"/>
            </c:if>

			     <c:if test="${ebOrder.onoffLineStatus!=3&&ebOrder.onoffLineStatus!=2}">
                     <fmt:formatNumber type="number" value="${ebOrder.orderAmount-ebOrder.lovePayAmount-ebOrder.frozenLovePayAmount}" pattern="0.00" maxFractionDigits="2"/>
                 </c:if></span></li>
