...
<head>
	...
	<script type="text/javascript">
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxsys}/Order/saleorderAftersalelist?orderId=${ebOrder.orderId}");
			$("#searchForm").submit();
	    	return false;
	    }
...
<body>
    <div class="c-context">
      <ul class="nav-ul" style="margin:0px">
          <li><a href="${ctxsys}/Order/saleorderdata?orderId=${ebOrder.orderId}" style="overflow: hidden;
    height: 40px;
    line-height: 40px;
    background: #ffffff;
    padding-left: 33px;
    margin-bottom: 20px;">订单资料</a></li>
          <li><a href="${ctxsys}/OrderLog/saleorderhis?orderId=${ebOrder.orderId}" style="overflow: hidden;
    height: 40px;
    line-height: 40px;
    background: #ffffff;
    padding-left: 33px;
    margin-bottom: 20px;">历史分析</a></li>
          <li><a href="${ctxsys}/Order/saleorderdeliverylog?orderId=${ebOrder.orderId}" style="overflow: hidden;
    height: 40px;
    line-height: 40px;
    background: #ffffff;
    padding-left: 33px;
    margin-bottom: 20px;">递送日志</a></li>
          <li><a class="active" href="${ctxsys}/Order/saleorderAftersalelist?orderId=${ebOrder.orderId}" style="overflow: hidden;
    height: 40px;
    line-height: 40px;
    background: #ffffff;
    padding-left: 33px;
    margin-bottom: 20px;
	border-bottom: 0px solid #69AC72;
	">订单退货处理</a></li>
          <li><a href="${ctxsys}/Order/saleorderdeliverymanager?orderId=${ebOrder.orderId}" style="overflow: hidden;
    height: 40px;
    line-height: 40px;
    background: #ffffff;
    padding-left: 33px;
	...
	">递送管理</a></li>
          <li><a href="${ctxsys}/Order/saleorderendreply?orderId=${ebOrder.orderId}" style="overflow: hidden;
    height: 40px;
    line-height: 40px;
    background: #ffffff;
    padding-left: 33px;
    margin-bottom: 20px;">最终批复</a></li>
      </ul>
</div>
  </div>
	 <form id="searchForm" action="${ctxsys}/Order/saleorderAftersalelist" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">
		    <li>
		    <label>退货编号</label>
		    <input id="saleNo" name="saleNo" type="text" htmlEscape="false" maxlength="20" style="height:30px" class="input-medium" value="${saleNo}"  placeholder="请输入退款编号"/></li>
			...
			<li><input id="btnSubmit" class="btn btn-primary" type="submit" style="margin-left:10px" value="查询" onclick="return page();"/></li>
			<li><input id="btnExport" class="btn btn-primary check-a1" type="button" style="display:none" value="导出"/></li>
		</ul>
			<div class="check1">
    <div class="check-box">
	...
	<table id="treeTable" class="table table-striped table-condensed table-bordered" >
		...
		<c:forEach items="${page.list}" var="AfterSale" varStatus="status">
			<tr>
			    ...
			    <td><a href="${ctxsys}/Order/saleorderreturngoods?orderId=${ebOrder.orderId}&saleId=${AfterSale.saleId}">${AfterSale.saleNo}</a></td>
				...
				<td><fmt:formatNumber value="${AfterSale.deposit+AfterSale.realFreight}" type="currency"/></td>
				...
				<td>
					<c:if test="${empty AfterSale.appGoodsMethod}">待确定</c:if>
		            <c:if test="${AfterSale.appGoodsMethod==1}">快递</c:if>
			    	<c:if test="${AfterSale.appGoodsMethod==2}">送至自提点</c:if>
			    	<c:if test="${AfterSale.appGoodsMethod==3}">上门取件</c:if>
				</td>
				...
				<td><a href="${ctxsys}/NcMessageTable/saleorderAfternc?orderId=${ebOrder.orderId}&saleId=${AfterSale.saleId}">查询同步</a></td>
			</tr>
		</c:forEach>
	</table>
