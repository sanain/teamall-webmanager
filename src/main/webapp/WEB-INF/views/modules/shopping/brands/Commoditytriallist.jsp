...
<body>
	<ul class="nav nav-tabs">
	 ...
	 <form:form id="searchForm1" modelAttribute="ebCommoditytrial" action="${ctxsys}/EbCommoditytria" method="post" class="breadcrumb form-search ">
		...
		<ul class="ul-form">
		    <li><form:input path="productNo" htmlEscape="false" maxlength="50" class="input-medium"  placeholder="请输入关键词"/></li>
			<li><form:select path="status"  htmlEscape="false" maxlength="50"  class="input-medium">
		      ...
		      <%--<li><form:input path="produtName" htmlEscape="false" maxlength="50" class="input-medium"  placeholder="请输入商品名字"/></li> --%>
			<li class="btns"><input id="btnSubmit" class="btn btn-primary" type="submit" value="查询" onclick="return page();"/>
		</ul>
	</form:form> 
	<tags:message content="${message}"/>
	<table id="treeTable" class="table table-striped table-condensed" border="1">
		<tr><th><span>编号</span></th><th>商品名称</th><th>优惠类型</th><th class="sort-column startTime">开始时间</th><th class="sort-column stopTime">结束时间</th><th>优惠价</th><th>数量</th><th>状态</th><shiro:hasPermission name="merchandise:sales:edit"><th>操作</th></shiro:hasPermission></tr>
		<c:forEach items="${page.list}" var="Commoditytrialist" varStatus="status">
			<tr>
			    <td>${status.index+1}</td>
				<td><a href="${ctxsys}/EbCommoditytria/form?commoditytrialId=${Commoditytrialist.commoditytrialId}">${Commoditytrialist.produtName}</a></td>
			  ...
			  <td> <shiro:hasPermission name="merchandise:sales:edit">
					<a href="${ctxsys}/EbCommoditytria/form?commoditytrialId=${Commoditytrialist.commoditytrialId}">修改</a>
					<a href="${ctxsys}/EbCommoditytria/delete?commoditytrialId=${Commoditytrialist.commoditytrialId}" onclick="return confirmx('要删除该区域及所有子区域项吗？', this.href)">删除</a>
				  </shiro:hasPermission>
				  ...
				  <a href="${ctxsys}/EbIndianarecord/list?objectId=${Commoditytrialist.commoditytrialId}">查看记录</a>
				    ...
				    <c:if test="${Commoditytrialist.type==4}">
				  <shiro:hasPermission name="merchandise:sales:edit">
				  <a href="${ctxsys}/EbIndianarecord/list?objectId=${Commoditytrialist.commoditytrialId}">查看记录</a>
				  </shiro:hasPermission>
				  </c:if>
