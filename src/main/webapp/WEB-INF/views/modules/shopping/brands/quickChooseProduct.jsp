...
<head>
    ...
    <script>
        function page(n, s) {
            if (n) $("#pageNo").val(n);
            if (s) $("#pageSize").val(s);
            $("#searchForm").attr("action", "${ctxsys}/Product/quickChooseProduct");
            $("#searchForm").submit();
            return false;
        }
    </script>
...
<div style="overflow:hidden;height:350px;">
    ...
    <div style="float:left;width:75%;overflow:scroll;height:100%;">
        <form:form id="searchForm" modelAttribute="ebProduct"
                   action="${ctxsys}/Product/quickChooseProduct" method="post"
...
<script type="text/javascript">
    ...
    function chooseAll() {
        ...
        $.ajax({
            url : "${ctxsys}/Product/selectAllProduct",
            type : "POST",
            data : $("#searchForm").serialize(),
            datatype : "json",
            success : function(data) {
                debugger
                $("#chooseIds").val(data.ids);
                $("#productNames").val(data.names);
