...
<head>
	...
	function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxsys}/PmProductTypeSpertAttr/list");
			$("#searchForm").submit();
	    	return false;
	    }
...
<body>
  <c:if test="${fulte==1}">
	...
	<li class="active"><a href="${ctxsys}/PmProductTypeSpertAttr/list?productTypeId=${productTypeId}">规格属性列表</a></li>
	<li>
	  <shiro:hasPermission name="merchandise:PmProductTypeSpertAttr:edit">
	   <a href="${ctxsys}/PmProductTypeSpertAttr/from?productTypeId=${productTypeId}">规格属性添加</a>
	  </shiro:hasPermission>
	</li>
	</ul>
	 <form:form id="searchForm" modelAttribute="sbProductTypeSpertAttr" action="${ctxsys}/PmProductTypeSpertAttr/list" method="post" class="breadcrumb form-search ">
		...
		<c:forEach items="${page.list}" var="sbProductTypeSpertAttrList" varStatus="status" >
			 <tr>
				...
				<td class="center123">
				 ...
				 <td class="center123">
					 <c:if test="${empty sbProductTypeSpertAttrList.isPublic||sbProductTypeSpertAttrList.isPublic==0}">平台使用</c:if>
					 <c:if test="${not empty sbProductTypeSpertAttrList.isPublic&&sbProductTypeSpertAttrList.isPublic==1}">商家使用</c:if>
					 <c:if test="${not empty sbProductTypeSpertAttrList.isPublic&&sbProductTypeSpertAttrList.isPublic==2}">平台和商家共用</c:if>
				 </td>
				<td class="center123">${sbProductTypeSpertAttrList.orderNo}</td>
			  <%--  <shiro:hasPermission name="merchandise:SbProductTypeBrand:edit"> --%>
			   	 <td class="center123">
			   	    <a href="${ctxsys}/PmProductTypeSpertAttrValue/list?spertAttrId=${sbProductTypeSpertAttrList.id}">明细</a>
			   	 	<a href="${ctxsys}/PmProductTypeSpertAttr/from?id=${sbProductTypeSpertAttrList.id}">修改</a>
					<a href="${ctxsys}/PmProductTypeSpertAttr/delete?id=${sbProductTypeSpertAttrList.id}" onclick="return confirmx('是否删除该条信息？', this.href)">删除</a>
				 </td>
				<%-- </shiro:hasPermission> --%>
			</tr>
		</c:forEach>
