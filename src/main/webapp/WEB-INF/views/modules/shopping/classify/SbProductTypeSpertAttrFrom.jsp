	...
	<script type="text/javascript">
		...
		function love(){
			...
			$.ajax({
				type: "POST",
				 async: false,
				url: "${ctxsys}/PmProductTypeSpertAttr/checkName?productTypeId="+${sbProductType.id}+"&spertAttrType="+ck+"&spertAttrName="+$('#spertAttrName').val()+"&id="+$('#id').val()+"",
...
<body style="height: 100%">
	 ...
	 <li ><a href="${ctxsys}/PmProductTypeSpertAttr/list?productTypeId=${productTypeId}">规格属性列表</a></li>
	 <li class="active"><a href="">规格属性<shiro:hasPermission name="merchandise:PmProductTypeSpertAttr:view">${not empty sbProductTypeSpertAttr.id?'修改':'添加'}
	 </shiro:hasPermission><shiro:lacksPermission name="merchandise:PmProductTypeSpertAttr:view">查看</shiro:lacksPermission></a></li>
	</ul><br/> 
	
	<form:form id="inputForm" modelAttribute="sbProductTypeSpertAttr" action="${ctxsys}/PmProductTypeSpertAttr/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
		 ...
		 <div class="control-group" style="display:none;">
			<label class="control-label" for="isShow" > 录入类型:</label>
			  <div class="controls">
		  ...
		  <div class="control-group">
			<label class="control-label" for="isShow">前台搜索显示:</label>
			  <div class="controls">
			 <!--  1为手工录入，2为从列表选择。当为规格时：必须从列表选择。当为属性时：手工录入为文本框，从列表选择为下拉框 -->
		   ...
		   <div class="control-group">
			<label class="control-label" for="isShow">是否后台显示:</label>
			  <div class="controls">
			 <!--  1为手工录入，2为从列表选择。当为规格时：必须从列表选择。当为属性时：手工录入为文本框，从列表选择为下拉框 -->
		...
		<div class="control-group">
			<label class="control-label" for="isShow">使用状态:</label>
			<div class="controls">
		...
		<div class="control-group">
			<label class="control-label" for="isShow">来源:</label>
			<div class="controls">
