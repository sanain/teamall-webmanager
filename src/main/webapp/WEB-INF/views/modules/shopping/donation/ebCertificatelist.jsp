...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxsys}/EbConversioncode/selectList");
			$("#searchForm").submit();
	    	return false;
	    }
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxsys}/EbConversioncode/selectList">优惠列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="ebConversioncode" action="${ctxsys}/EbConversioncode/selectList" method="post" class="breadcrumb form-search ">
		...
		<li><input name="ids" value="${ids}" maxlength="50"  htmlEscape="false"/> </li>
	...
	<table id="treeTable" class="table table-striped table-condensed">
		...
		<c:forEach items="${page.list}" var="ebConversioncodeList" varStatus="status">
			<tr>
				...
				<td><a href="${ctxsys}/EbConversioncode/form?id=${ebConversioncodeList.id}">${ebConversioncodeList.redcontainingName}</a></td>
