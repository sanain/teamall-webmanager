...
<head>
	...
	<script type="text/javascript">
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxweb}/ebWarehouseItem/ebWarehouseorderlist");
			$("#searchForm").submit();
	    	return false;
	    }
	    $(function(){
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxweb}/ebWarehouseItem/ebWarehouseorderlist">派送列表</a></li>
		<li><a href="${ctxweb}/ebWarehouseItem/saleorderlist" style="display:none">售后列表</a></li>
	</ul>
	 <form:form id="searchForm" modelAttribute="ebOrder" action="${ctxweb}/ebWarehouseItem/ebWarehouseorderlist" method="post" class="breadcrumb form-search ">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
	    <input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}" />
		<tags:tableSort id="orderBy" name="orderBy" value="${page.orderBy}" callback="page();"/>
		<ul class="ul-form">
		     <input type="hidden" name="id" value="${ebWarehouseuser.id}">
		    <li>
		     <label>订单编号</label>
	...
	<table id="treeTable" class="table table-striped table-condensed table-bordered" >
	    ...
	    <th class="sort-column payTime ">下单时间 </th>
		 <th class="sort-column wareOptionTime ">指派时间 </th>
	    <th>操作 </th>
		</tr>
		<c:forEach items="${page.list}" var="ebOrder" varStatus="status">
			<tr>
				...
				<td> 
				     ${ebOrder.wareOptionTime} 
				</td>
				
				<td> 
				    <a href="${ctxweb}/ebWarehouseItem/ebWarehouseorderform?orderId=${ebOrder.orderId}" target="tager">详细</a>
				</td>
			</tr>
		</c:forEach>
	</table>
