...
<head>
	...
	<script>
        function page(n, s) {
            if (n) $("#pageNo").val(n);
            if (s) $("#pageSize").val(s);
            $("#searchForm").attr("action", "${ctxweb}/shop/ebProductChargingApply/chooseProductCharging");
            $("#searchForm").submit();
            return false;
        }
	</script>
...
<form:form id="searchForm" modelAttribute="ebProductCharging" action="${ctxweb}/shop/ebProductChargingApply/chooseProductCharging" method="post" class="breadcrumb form-search ">
	...
	<ul class="ul-form">
		...
		<li><label></label><input id="btnSubmit" style="background: #4778C7;" class="btn btn-primary" type="submit" value="查询" onclick="return page();"/></li>
	</ul>
</form:form>
...
<div style="width: 100%">
	<input class="btn btn-primary" style="margin:0px 0px 5px 5px ;background: #4778C7;width: 37px;font-size: 11px;padding: 1px 0px 0px 1px;height: 25px;" id="choose-all-btn" status="0" type="button" value="全选" onclick="return chooseAll();"/>
</div>
...
<script type="text/javascript">
    ...
    function chooseAll(){
        ...
        $.ajax({
            url:"${ctxweb}/shop/ebProductChargingApply/chooseAll?productTypeId="+$("#productTypeId").val(),
            type:"POST",
            datatype:"json",
