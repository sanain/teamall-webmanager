...
<head>
	...
	function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxweb}/shop/PmProductTypeSpertAttrValue/list");
			$("#searchForm").submit();
	    	return false;
	    }
...
<body>
	...
	<li class="active"><a href="${ctxweb}/shop/PmProductTypeSpertAttrValue/list?spertAttrId=${spertAttrId}">规格属性明细列表</a></li>
	<li>
		<%-- <shiro:lacksPermission name="merchandise:PmProductTypeSpertAttrValue:edit"> --%>
		   <a href="${ctxweb}/shop/PmProductTypeSpertAttrValue/from?spertAttrId=${spertAttrId}">规格属性明细添加</a>
		<%-- </shiro:lacksPermission> --%>
	</li>
	</ul>
	 <form:form id="searchForm" modelAttribute="sbProductTypeSpertAttrValue" action="${ctxweb}/shop/PmProductTypeSpertAttrValue/list" method="post" class="breadcrumb form-search ">
		...
		<c:forEach items="${page.list}" var="sbProductTypeSpertAttrValueList" varStatus="status" >
			 <tr>
				...
				<td  class="center123">${sbProductTypeSpertAttrValueList.orderNo}</td>
			   	 <td  class="center123">
			   	 	<a href="${ctxweb}/shop/PmProductTypeSpertAttrValue/from?id=${sbProductTypeSpertAttrValueList.id}">修改
					</a>
					<a href="${ctxweb}/shop/PmProductTypeSpertAttrValue/delete?id=${sbProductTypeSpertAttrValueList.id}" onclick="return confirmx('是否删除该条信息？', this.href)">删除</a>
