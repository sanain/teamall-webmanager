...
<head>
    ...
    <script type="text/javascript">
        ...
        function page(n, s) {
            ...
            if (n) $("#pageNo").val(n);
            if (s) $("#pageSize").val(s);
            $("#searchForm").attr("action", "${ctxweb}/shop/ticket/ticketList");
            $("#searchForm").submit();
            return false;
        }
    </script>
    <script type="text/javascript">
        ...
        function show1(ebTicketId) {
            ...
            $.ajax({
                type : "POST",
                url : "${ctxweb}/shop/ticket/ebTicketReplace", //请求的url地址
                data:{id:ebTicketId},
        ...
        // 显示弹框
        function show(ebTicketType,ebTicketId,title,state) {
        ...
        function state() {
            ...
            $.ajax({
                type: "post",
                url: "${ctxweb}/shop/ticket/stateTicket",
                data: params,
                beforeSend: function () {
        ...
        function del() {
            ...
            $.ajax({
                type: "post",
                url: "${ctxweb}/shop/ticket/delTicket",
                data: params,
                beforeSend: function () {
...
<div class="ibox-content">
    <ul class="nav nav-tabs">
        ...
        <li><a href="${ctxweb}/shop/ticket/ticketForm">添加模板</a></li>
    </ul>
    <table class="table table-striped table-bordered table-hover dataTables-example">
        ...
        <c:forEach items="${page.list}" var="ebTickets" varStatus="status">
            <tr>
                ...
                <th><a onclick="show('2','${ebTickets.id}','','${ebTickets.state}')" style="margin-left: 10px;cursor:pointer;"><c:if test="${ebTickets.state==0}">禁用</c:if><c:if test="${ebTickets.state==1}">启用</c:if></a></th>
                <th><a onclick="show('1','${ebTickets.id}','确定删除${ebTickets.modelName}模板？','')" style="color:red;cursor:pointer;">删除</a>
                    <a onclick="show1('${ebTickets.id}')" style="margin-left: 10px;color:red;cursor:pointer;">预览</a>
                </th>
            </tr>
        </c:forEach>
        </tbody>
