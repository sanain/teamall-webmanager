...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctxsys}/sys/office/list?isAgent=0">机构列表</a></li>
		<li class="active"><a href="${ctxsys}/sys/office/form?id=${office.id}&parent.id=${office.parent.id}">机构<shiro:hasPermission name="sys:office:edit">${not empty office.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:office:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	
	<form:form id="inputForm" modelAttribute="office" action="${ctxsys}/sys/office/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
                <tags:treeselect id="office" name="parent.id" value="${office.parent.id}" labelName="parent.name" labelValue="${office.parent.name}"
					title="机构" url="${ctxsys}/sys/office/treeData?type=1&isAgent=0" extId="${office.id}" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
