...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$("#inputForm").validate({
				rules: {
					name: {remote: "${ctxsys}/sys/role/checkName?oldName=" + encodeURIComponent("${role.name}")}
				},
				messages: {
					name: {remote: "角色名已存在"}
				},
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctxsys}/sys/role/">角色列表</a></li>
		<li class="active"><a href="${ctxsys}/sys/role/form?id=${role.id}">角色<shiro:hasPermission name="sys:role:edit">${not empty role.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:role:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	
	<form:form id="inputForm" modelAttribute="role" action="${ctxsys}/sys/role/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
