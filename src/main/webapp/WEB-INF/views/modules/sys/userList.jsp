...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxsys}/sys/user/list");
			$("#searchForm").submit();
	    	return false;
	    }
	</script>
		...
		<form id="importForm" action="${ctxsys}/sys/user/import" method="post" enctype="multipart/form-data"
			...
			<a href="${ctxsys}/sys/user/import/template">下载模板</a>
				...
				<ul class="nav nav-tabs">
					...
					<shiro:hasPermission name="sys:user:edit"><li><a href="${ctxsys}/sys/user/form">用户添加</a></li></shiro:hasPermission>
				</ul>
					<form:form id="searchForm"  modelAttribute="sysUser" action="${ctxsys}/sys/user/list" method="post" class="breadcrumb form-search ">
					...
					<div class="p-xs">
						<ul class="ul-form">
							<li><label>归属公司：</label><tags:treeselect id="company" name="company.id" value="${sysUser.company.id}" labelName="company.name" labelValue="${sysUser.company.name}" 
								title="公司" url="${ctxsys}/sys/office/treeData?type=1&isAgent=0" cssClass="input-small" allowClear="true" /></li>
							...
							<li><label>归属部门：</label><tags:treeselect id="office" name="office.id" value="${sysUser.office.id}" labelName="office.name" labelValue="${sysUser.office.name}" 
								title="部门" url="${ctxsys}/sys/office/treeData?type=2&isAgent=0" cssClass="input-small" allowClear="true" notAllowSelectParent="true"/></li>
                        ...
                        <table class="table table-striped table-bordered table-hover dataTables-example" >
                            ...
                            <c:forEach items="${page.list}" var="user">
								<tr class="gradeX">
									...
									<td><a href="${ctxsys}/sys/user/form?id=${user.id}">${user.loginName}</a></td>
									<td>${user.name}</td>
									<td>${user.phone}</td>
									<td>${user.mobile}</td>
									<td>${user.roleNames}</td>
									<shiro:hasPermission name="sys:user:edit"><td>
					    				<a href="${ctxsys}/sys/user/form?id=${user.id}">修改</a>
