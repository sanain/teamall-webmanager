...
<head>
	...
	<script type="text/javascript">
    	...
    	function updateSort() {
			loading('正在提交，请稍等...');
	    	$("#listForm").attr("action", "${ctxsys}/cms/category/updateSort");
	    	$("#listForm").submit();
    	}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxsys}/cms/category/">栏目列表</a></li>
		<shiro:hasPermission name="cms:category:edit"><li><a href="${ctxsys}/cms/category/form">栏目添加</a></li></shiro:hasPermission>
	</ul>
		...
		<table id="treeTable" class="table table-striped table-bordered table-condensed">
			...
			<c:forEach items="${list}" var="tpl">
				<tr id="${tpl.id}" pId="${tpl.parent.id ne '1' ? tpl.parent.id : '0'}">
					<td><a href="${ctxsys}/cms/category/form?id=${tpl.id}">${tpl.name}</a></td>
					...
					<td>
						...
						<shiro:hasPermission name="cms:category:edit">
							<a href="${ctxsys}/cms/category/form?id=${tpl.id}">修改</a>
							<a href="${ctxsys}/cms/category/delete?id=${tpl.id}" onclick="return confirmx('要删除该栏目及所有子栏目项吗？', this.href)">删除</a>
							<a href="${ctxsys}/cms/category/form?parent.id=${tpl.id}">添加下级栏目</a>
						</shiro:hasPermission>
					</td>
				</tr>
			</c:forEach>
