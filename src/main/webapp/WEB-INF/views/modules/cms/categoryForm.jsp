...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctxsys}/cms/category/">栏目列表</a></li>
		<li class="active"><a href="${ctxsys}/cms/category/form?id=${category.id}&parent.id=${category.parent.id}">栏目<shiro:hasPermission name="cms:category:edit">${not empty category.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cms:category:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	
	<form:form id="inputForm" modelAttribute="category" action="${ctxsys}/cms/category/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			...
			<div class="controls">
                <tags:treeselect id="office" name="office.id" value="${category.office.id}" labelName="office.name" labelValue="${category.office.name}"
					title="机构" url="${ctxsys}/sys/office/treeData" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
                <tags:treeselect id="category" name="parent.id" value="${category.parent.id}" labelName="parent.name" labelValue="${category.parent.name}"
					title="栏目" url="${ctxsys}/cms/category/treeData" extId="${category.id}" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
