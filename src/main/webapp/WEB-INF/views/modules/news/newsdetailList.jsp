...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxsys}/news/newsdetail/list");
			$("#searchForm").submit();
	    	return false;
	    }
		 ...
		 function sss(){
			 $.ajax({
					url : "${ctxsys}/news/newsdetail/blobToClob",
					type : 'post',
					data : {
					},
					cache : false,
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctxsys}/news/newsdetail/list">站内资讯内容列表</a></li>
		<shiro:hasPermission name="news:newsdetail:edit"><li><a href="${ctxsys}/news/newsdetail/form?newstypeid.newstypeid=${newstypeid}">站内资讯内容添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="newsdetail" action="${ctxsys}/news/newsdetail/list" method="post" class="breadcrumb form-search ">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th><input type="checkbox"  id="allchkbtn" /></th><th>栏目</th><th>分类</th><th>状态</th><th>标题</th><th>作者</th><th>引用（来自）</th><th>是否热点</th><th>创建时间</th><th>发布时间</th><shiro:hasPermission name="news:newsdetail:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="newsdetail">
			<tr>
				...
				<shiro:hasPermission name="news:newsdetail:edit"><td>
    				<a href="${ctxsys}/news/newsdetail/form?newsdetailid=${newsdetail.newsdetailid}">修改</a>
    				<a href="${ctxsys}/news/newsdetail/picform?newsdetailid.newsdetailid=${newsdetail.newsdetailid}&fromType=list">添加相关图片</a>
    				<a href="${ctxsys}/news/newsdetail/delete?id=${newsdetail.newsdetailid}&newstypeid=${newstypeid}" onclick="return confirmx('确认要删除该文章吗？', this.href)">删除</a>
