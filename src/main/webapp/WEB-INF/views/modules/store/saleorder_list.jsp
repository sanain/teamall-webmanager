...
<head>
	...
	<script type="text/javascript">
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action","${ctxweb}/storeItem/saleorderlist");
			$("#searchForm").submit();
	    	return false;
	    }
	    $(function(){
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctxweb}/storeItem/storeorderlist">派送列表</a></li>
		<li class="active"><a href="${ctxweb}/storeItem/saleorderlist">售后列表</a></li>
	</ul>
	 <form:form id="searchForm" modelAttribute="ebAftersale" action="${ctxweb}/storeItem/saleorderlist" method="post" class="breadcrumb form-search ">
	...
	<table id="treeTable" class="table table-striped table-condensed table-bordered" >
		...
		<c:forEach items="${page.list}" var="ebAftersaleList" varStatus="status">
            ...
            <c:when test="${ebAftersaleList.refundStatus==9}">
				...
				<td> 
				    <a href="${ctxweb}/storeItem/saleorderform?saleId=${ebAftersaleList.saleId}" target="tager">详细</a>
				</td>
			</tr>
		</c:forEach>
	</table>
