...
<%@ attribute name="isDecDate" type="java.lang.Boolean" required="false" description="是否不加日期"%>
...
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		...
		var isDecDate='${isDecDate}';
		if(isDecDate=='true')
		yearMonth='';
		var url = "${ctxStatic}/ckfinder/ckfinder.html?type=${ctype}&start=${ctype}:${uploadPath}/"+yearMonth+"&action=js&func=${input}SelectAction&thumbFunc=${input}ThumbSelectAction&cb=${input}Callback&dts=${type eq 'thumb'?'1':'0'}&sm=${selectMultiple?1:0}";
	...
	function ${input}Callback(api){
	    ...
	    api.disableFileContextMenuOption('FileContextMenu_resizeImage', false);  
