...
//					特定class的input不能为空
					cbNull:function(className,_thisClassName){
						...
						this.each(function() {
							...
							$(this).closest('form').find('.' + className).each(function () {
								/*添加placeholder提示语句*/
								$(this).parents('form').find('.' + className).each(function () {
									if (!$(this).is(":hidden")&&($(this).attr("value") == '' || $(this).val().length == 0)) {
										$(this).parents('form').find("." + className).attr("index", "0");
										$(this).attr("placeholder", "该区域不能为空");
										nu2 = 1
									}
								});
...
//					数字
					cbNum:function(){
						var str = /^\d+\.{0,1}\d*$/;/*只能以数字开头和结尾,只能输入数字*/
						var str1 = /[^\d]/g;
						baseTest(str,this,str1);
						return this;
					},
					cbNum2:function(){
						var str = /\d+(\.\d{0,2})?/;/*只能以数字开头和结尾,只能输入数字*/
						var str1 = /[^\d]{0,2}/g;
						baseTest(str,this,str1);
						return this;
					},
