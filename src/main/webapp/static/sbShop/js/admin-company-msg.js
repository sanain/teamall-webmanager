$(function(){
    $('.sub').click(function(){
    	...
    	$(this).parents('form').find('input[type=text],input[type=number],textarea[type=text]').css("border", "1px solid #a9a9a9");
    	if(legalPerson==""){
			...
			var phone = msgPhone.split(",");
			for(var i = 0 ; i < phone.length ; i++){
				if(phone[i]!=""&&!/^[1][345678][0-9]{9}$/.test(phone[i])){
					$("#msgPhone").css("border", "1px solid red").attr("placeholder","请检查短信通知手机！");
					alert("请检查短信通知手机！");
					return false;
				}
			}
