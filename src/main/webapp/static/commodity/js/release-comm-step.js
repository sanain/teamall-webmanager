var commonNum='';
function addboft() {
	alert($('#add-stan option:selected').val());
	$.ajax({
		type : "POST",
		url : ctxsysctxsys + "/Product/commercialeDetail",
		data : {
			id : $('#add-stan option:selected').val(),
			shopId : $("#advertiseTypeObjIds").val(),
			productId : $("#kk").val()
		},
		success : function(data) {
			...
			attval += "<a data-did='9' href='javascript:;'><img src=" + contextPath + "'/sbShop/images/add-a.png' alt=''></a>"
...
$(function() {
	...
	$('body').on('blur', '.t-ben', function() {
		...
		//
		// }else{
		// 	$('.t-shi').css('border','1px solid #a9a9a9');
		// }
	...
	//
	// }else{
	// 	$('.bshi').css('border','1px solid #DCDCDC');
	// }
	});
	...
	$('body').on('input propertychange', '.tt-ran', function() {
		...
		if ($(this).val().length >= $('.ran1').text().length) {
			if (!isNaN(ben) && !isNaN(ran)) {
				...
				$('.tt-jie').val('')
				console.log('ran' + ran + "val2" + val2);
				if (ran <= val2) {
					var tyr = $(this).closest('table').find('tbody').find('tr');

					for (h = 0; h < tyr.length; h++) {
						console.log('tyr:' + tyr[h] + ",tyr" + $(tyr[h]).find('.t-ben').val());
	...
	$('body').on('click', '.step3', function() {
		...
		if (auditState != 0) {
			...
			//计量类型为件时，判断规格
			if ($("input[name='measuringType']:checked").val() == 1) {
			...
			if (false && $('.maijia-ul li:nth-child(1) input').is(":checked")) {
			...
			if (false && $('.shop-news ul li:nth-child(2) .radio-div:nth-child(3) input').is(':checked')) {
			...
			if ($('#advertiseTypeObjIds').val() == "") {
				$('.yanzhen-box span').text('请先选择门店');
				$('.yanzhen').show();
			} else {
				var sele = $('body select');
				var selen = 0;
				var tran = [];
				for (u = 0; u < $('t-ran').length; u++) {
					tran.push($($('t-ran')[u]).val());
				}



				if ($('.FreightTem').siblings('input').is(':checked')) {

				} else {

					for (h = 0; h < sele.length; h++) {
						if ($(sele[h]).hasClass('FreightTem')) {
							selen = h;
				...
				if (selen != 0) {
					var akss = sele.splice(selen, 1);

				}

				if ($('.shop-news ul li:nth-child(2) input:checked').val() == 2 || ($('.shop-news ul li:nth-child(3) input:checked').val() == 1)) {
					if ($(this).cbNull('input', 'step3') == 1) {

						$('.yanzhen-box span').text('有选项没有填完，请继续填写！');
						$('.yanzhen').show();
					} else {
						for (i = 0; i < sele.length; i++) {

							if ($(sele[i]).css('display') == 'none') {

							} else if ($(sele[i]).is(":hidden")) {

							} else {
								if ($(sele[i]).attr('id') == 'shopType') {

								} else if ($(sele[i]).attr('name') == 'standarBraind_id') {

								} else if ($(sele[i]).attr('name') == 'standard_name') {

								} else {
									if ($(sele[i]).closest('.cke_dialog').length == 0) {
						...
						if ($('.ssa').cbNull('input', 'ssa') == 1) {
							console.log('7');
							$('.yanzhen-box span').text('有选项没有填完，请继续填写！');
							$('.yanzhen').show();
						}

						if ($('#prdouctImgPreview li img').length == 0) {
							$('.yanzhen-box div span').text('请添加商品主图。');
							$('.yanzhen').show();
							return
						}
						if ($('.property-div ul .radio-div').length != 0) {
							for (m = 0; m < $('.property-div ul').length; m++) {
								var roperty = 0;
								if ($($('.property-div ul')[m]).children('li').children('.radio-div').length != 0) {
									var mmv = $($('.property-div ul')[m]).find('.radio-div');
						...
						$.ajax({
							...
							success : function(data) {
								...
								if (data.code == '05') {
									...
									$('.yanzhen').show();
									location.href = ctxsysctxsys + "/Product/Orderlist?isLovePay=1";
									...
									$('.yanzhen-box div span').text(data.msg);
									$('.yanzhen').show();
									location.href = ctxsysctxsys + "/Product/list?shopType=" + data.shopType;
								} else if (data.code == '009') {
									$('.yanzhen-box div span').text(data.msg);
									$('.yanzhen').show();
									location.href = ctxsysctxsys + "/Product/supplylist";
								}
							}
						});
					...
					// 	if(parseFloat($(btben[x]).val())>parseFloat($(btshi[x]).val())){
					// 		$('.yanzhen-box div span').text('市场价格必须大于本店价格！');
					// 		$('.yanzhen').show();
					// 		return;
					// 	}
						...
						for (i = 0; i < sele.length; i++) {
							if ($(sele[i]).css('display') == 'none') {
								...
								if ($(sele[i]).attr('id') == 'shopType') {
									...
									if ($(sele[i]).find('option:selected').val() == "") {
										if ($(sele[i]).attr('name') == 'standarBraind_id') {
											continue
										} else if ($(sele[i]).attr('name') == 'standard_name') {
											continue
										}
	...
	$("body").on('click','#setting',function(){
		var arr=[];
		for(var i=0;i<$(".guige-div ul li").length;i++){
			var value=$(".guige-div ul li").eq(i).find('.select option:selected').val();
			if(value!="" &&value!="undefined"){
				arr.push(value);
			}
		}
		
		var nth=$('.table-div table tbody tr td:nth-child(1)');
//		$('.table-div table tbody tr td').find("input[name='detail_prices']").val("");
//		$('.table-div table tbody tr td').find("input[name='member_price']").val("");
//		$('.table-div table tbody tr td').find("input[name='detailInventory']").val("");
		
		
		var price1=$("#price1").val();// 本店价格
		var price2=$("#price2").val();// 会员价格
		var price3=$("#price3").val();// 会员价格
		for(var j=0;j<nth.length;j++){
			var value=nth.eq(j).find("input").val();
			var num=1;
			for(var k=0;k<arr.length;k++){
				if(value.indexOf(arr[k])==-1){
					num=-1;
					break;
				}
				else{
			...
			if(num==1){				
				$(nth[j]).closest("tr").find("td").find("input[name='detail_prices']").val(price1);
				$(nth[j]).closest("tr").find("td").find("input[name='member_price']").val(price2);
				$(nth[j]).closest("tr").find("td").find("input[name='detailInventory']").val(price3);
			}
		}
	})
            ...
            $(".choose-price").show();
				...
				$.ajax({
					...
					success : function(date) {
						...
						html2 += `<a href="javascript:;"><img src="${contextPath}/static/sbShop/images/add-a.png" alt=""></a></li></ul>`;
						...
						if ($('.standard-left input[type=checkbox]:checked').length == 1) {
							...
							if (date.length != 0) {
								var tshi = $('.t-shi').val();
								var ttben = $('.tt-ben').val();
								var ttran = $('.tt-ran').val();
								var tjie = $('.t-jie').val();
								var huiy = $('.t-huiy').val();
								...
								if ($('.standard-left input[type=checkbox]:checked').length == 2) {
									...
									for (var f = 0; f < ssp.length; f++) {
										for (var d = 1; d < date.length; d++) {
											...
											if (tshi == '') {
												ttm += `<td style="display: none"><input class="num t-shi" name="market_price" type="text"></td>`;
											} else {
												ttm += `<td style="display: none"><input class="num t-shi" name="market_price" type="text" value="${tshi}"></td>`;
											}
											if (ttben == '') {
												ttm += `<td><input class="num t-ben" name="detail_prices" type="text"></td>`;
											} else {
												ttm += `<td><input class="num t-ben" name="detail_prices" type="text" value="${ttben}"></td>`;
											}
											if (ttran == '') {
												ttm += `<td  style="display: none"><input class="num t-ran" name="return_ratio" type="text"></td>`;
											} else {
												ttm += `<td  style="display: none"><input class="num t-ran" name="return_ratio" type="text" value="${ttran}"></td>`;
											}
											if (tjie == '') {
												ttm += `<td  style="display: none"><input readonly class="num t-jie" name="supply_price" type="text"></td>`;
											} else {
												ttm += `<td  style="display: none"><input readonly class="num t-jie" name="supply_price" type="text" value="${tjie}"></td>`;
											}
											if (huiy == '') {
												ttm += `<td><input class="num t-huiy" name="member_price" type="text"></td>`;
											} else {
												ttm += `<td><input class="num t-huiy" name="member_price" type="text" value="${huiy}"></td>`;
											}
											if (tlast == '') {
												ttm += `<td><input class="num" name="detailInventory" type="text"></td></tr>`;
											} else {
												ttm += `<td><input class="num" name="detailInventory" type="text" value="${tlast}"></td></tr>`;
											}
										}
									}
								...
								if ($('.standard-left input[type=checkbox]:checked').length == 4) {

									colspan();
									// $('tfoot tr td:nth-child(1)').attr('colspan','4');
									var ssp1 = $($('.right-ul1')[0]).find('span');
									var ssp2 = $($('.right-ul1')[1]).find('span');
									var ssp3 = $($('.right-ul1')[2]).find('span');
									$('.table-div table tbody tr td:nth-child(3)').after(`<td data-t="${date[0].id}">${date[0].spertAttrValue}</td>`);
									var nth1 = $('.table-div table tbody tr td:nth-child(1)');
									for (c = 0; c < nth1.length; c++) {
										$(nth1[c]).find('input').val($(nth1[c]).find('input').val() + ':' + date[0].id);
									}
									for (var f = 0; f < ssp1.length; f++) {
										for (var g = 0; g < ssp2.length; g++) {
											for (var y = 0; y < ssp3.length; y++) {
												for (var d = 1; d < date.length; d++) {
													ttm += `<tr><td data-t="${$(ssp1[f]).find('input').val()}">${$(ssp1[f]).text()}<input name="standard_id_str" type="hidden" value="${$(ssp1[f]).find('input').val() + ':' + $(ssp2[g]).find('input').val() + ':' + $(ssp3[y]).find('input').val() + ':' + date[d].id}"></td><td data-t="${$(ssp2[g]).find('input').val()}">${$(ssp2[g]).text()}</td><td data-t="${$(ssp3[y]).find('input').val()}">${$(ssp3[y]).text()}</td><td data-t="${date[d].id}">${date[d].spertAttrValue}</td>`;
													if (tshi == '') {
														ttm += `<td style="display: none"><input class="num t-shi" name="market_price" type="text"></td>`;
													} else {
														ttm += `<td style="display: none"><input class="num t-shi" name="market_price" type="text" value="${tshi}"></td>`;
				...
				//移除价格变量
				var tteText = '#guige-'+$(this).val();
				$(tteText).parent().remove();
				var tte = $(this).val();
	...
	$('body').on('click', '.shop-news ul li:nth-child(3) input', function() {
		...
		if ($(this).val() == 1) {
			...
			$("#shop-standard").show();
			$(".statendTo").hide();
			if ($("input:radio[name='isHehuiguan']:checked").val() == "1") {
				$("input:radio[name='isHehuiguan']:eq(0)").attr("checked", 'checked');
				$("#hehuiUrl").val("");
				$("#hehuiurlDiv").css("display", "none");
			}
		}
	})
	...
	$('body').on('click', '.right-ul1  li b', function() {
		...
		var tteText=$(this).parent().parent().siblings(":first").find("input").val();
		var bt = $(this);
		$.ajax({
			type : "POST",
			url : ctxsysctxsys + "/Product/deletecommercialeDehors",
			data : {
				typeId : bid,
				productId : $("#kk").val()
			},
				...
				if (date.code == '00') {
					...
					if (tnum == 1) {
						if (lasp == 1) {
							...
							//移除价格变量
							var guigetteText = '#guige-'+tteText;
							$(guigetteText).parent().remove();
						...
						if (lasp == 1) {
							...
							$('tbody td[data-t=' + ta + ']').remove();
							$('.table-div table thead th:nth-child(4)').remove();
	...
	function colspan() {
		var colspan = 0;
		var length=$('.guige-div input[type=checkbox]:checked').length;
		if (length > 0) {
		...
		if (length > 3) {
			if ($($('.right-ul1')[3]).find('span').length > 0) {
				colspan = colspan + 1;
			}
		}
		$('tfoot tr td:nth-child(1)').attr('colspan', colspan + '');
	...
	$('body').on('click', '.right-ul1 a', function() {
		commonNum=$(this).parent().siblings(":first").find("input").val();
		th = $(this);
		sib = th.siblings();
	...
	$('.guige .guige-box-add').click(function() {
		...
		$.ajax({
			type : "POST",
			url : ctxsysctxsys + "/Product/addcommercialeDehors",
			data : {
				values : txt,
				typeId : snv,
				shopId : $("#advertiseTypeObjIds").val(),
			...
			success : function(date) {
				...
				html2 += `<a href="javascript:;"><img src="${contextPath}/static/sbShop/images/add-a.png" alt=""></a>`;
				...
				if ($('.right-ul1').length == 1) {								
					if ($('.right-ul1').find('span').length == snu) {
						$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id}"></td>
	                            <td><input class="num t-shi" name="market_price" type="text"></td>
	                            <td><input class="num t-ben" name="detail_prices" type="text"></td>
	                            <td><input class="num t-ran" name="return_ratio" type="text"></td>
	                              <td ><input readonly class="num t-jie" name="supply_price" type="text"></td>
	                          <td ><input class="num t-huiy" name="member_price" type="text"></td>
	                              <td><input class="num" name="detailInventory" type="text"></td>
	                             </tr>`);
					} else {
							...
							if ($($('.right-ul1')[0]).find('span').length == 1) {
								...
								if ($($('.right-ul1')[1]).find('span').length != 0) {
									...
									for (s = 0; s < sp1.length; s++) {
										$('.table-div table tbody').append(`<tr><td data-t="${date[0].id}">${date[0].spertAttrValue}<input name="standard_id_str" type="hidden" value="${date[0].id + ':' + $(sp1[s]).find('input').val()}"></td>
								...
								if (addspan.length != 0) {
									...
									for (x = 0; x < addspan.length; x++) {
										hp += `<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan[x]).find('input').val()}"></td><td data-t="${$(addspan[x]).find('input').val()}">${$(addspan[x]).text()}</td>
													<td><input class="num t-shi" name="market_price" type="text"></td>
													<td><input class="num t-ben" name="detail_prices" type="text"></td>
													<td><input class="num t-ran" name="return_ratio" type="text"></td>
													  <td><input readonly class="num t-jie" name="supply_price" type="text"></td>
									...
									$('.table-div table tbody').append(hp);
						...
						if ($($('.right-ul1')[0]).find('span').length == snu) {
							if ($($('.right-ul1')[0]).find('span').length == 1) {
								if (addspana1 == 0 && addspana2 == 0) {
									...
									$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id}"></td><td><input class="num t-shi" name="market_price" type="text"></td>
	                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
	                              		 <td ><input class="num t-ran" name="return_ratio" type="text"></td>
	                                	 <td ><input readonly class="num t-jie" name="supply_price" type="text"></td>
	                                	 <td ><input class="num t-huiy" name="member_price" type="text"></td>
								...
								if ($($('.right-ul1')[1]).find('span').length != 0 && $($('.right-ul1')[2]).find('span').length != 0) {
									for (x = 0; x < addspan1.length; x++) {
										for (z = 0; z < addspan2.length; z++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val()}"></td><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}</td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
								...
								} else if ($($('.right-ul1')[2]).find('span').length != 0) {
									for (x = 0; x < addspan2.length; x++) {
										$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${$(date[date.length - 1]).id + ':' + $(addspan2[x]).find('input').val()}"></td><td data-t="${$(addspan2[x]).find('input').val()}">${$(addspan2[x]).text()}</td>
												 <td><input class="num t-shi" name="market_price" type="text"></td>
												 <td><input class="num t-ben" name="detail_prices" type="text"></td>
												 <td ><input class="num t-ran" name="return_ratio" type="text"></td>
												 <td><input readonly class="num t-jie" name="supply_price" type="text"></td>
				...
				} else if ($('.right-ul1').length == 4) {
					if (uu == 1) {
						var addspan1 = $($('.right-ul1')[1]).find('span');
						var addspan2 = $($('.right-ul1')[2]).find('span');
						var addspan3 = $($('.right-ul1')[3]).find('span');
						var addspana1 = $($('.right-ul1')[1]).find('span').length;
						var addspana2 = $($('.right-ul1')[2]).find('span').length;
						var addspana3 = $($('.right-ul1')[3]).find('span').length;
						if ($($('.right-ul1')[0]).find('span').length == snu) {
							if ($($('.right-ul1')[0]).find('span').length == 1) {
								if (addspana1 == 0 && addspana2 == 0 && addspana3 == 0) {
									var trt2 = $($('.right-ul1')[1]).children('li:nth-child(1)').text();
									...
									// $('.table-div table tfoot tr td:nth-child(1)').attr('colspan','1')
									colspan();
								} else if (addspana1 != 0 && addspana2 != 0 && addspana3 == 0) {
									for (x = 0; x < addspan1.length; x++) {
										for (z = 0; z < addspan2.length; z++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val()}"></td><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}</td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
									...
									for (x = 0; x < addspan1.length; x++) {
										for (z = 0; z < addspan3.length; z++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan3[z]).find('input').val()}"></td><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}</td>
											 <td data-t="${$(addspan3[z]).find('input').val()}">${$(addspan3[z]).text()}</td>
		                              		 <td ><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                              		 <td ><input class="num t-ran" name="return_ratio" type="text"></td>
		                                	 <td ><input readonly class="num t-jie" name="supply_price" type="text"></td>
									...
									for (x = 0; x < addspan2.length; x++) {
										for (z = 0; z < addspan3.length; z++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan2[x]).find('input').val() + ':' + $(addspan3[z]).find('input').val()}"></td><td data-t="${$(addspan2[x]).find('input').val()}">${$(addspan2[x]).text()}</td>
											 <td data-t="${$(addspan3[z]).find('input').val()}">${$(addspan3[z]).text()}</td>
		                              		 <td ><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                              		 <td ><input class="num t-ran" name="return_ratio" type="text"></td>
									...
									for (y = 0; y < addspan1.length; y++) {
										for (x = 0; x < addspan2.length; x++) {
											for (z = 0; z < addspan3.length; z++) {
												$('.table-div table tbody').append(`<tr>
												<td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan1[y]).find('input').val() + ':' + $(addspan2[x]).find('input').val() + ':' + $(addspan3[z]).find('input').val()}"></td>
												<td data-t="${$(addspan1[y]).find('input').val()}">${$(addspan1[y]).text()}</td>
												<td data-t="${$(addspan2[x]).find('input').val()}">${$(addspan2[x]).text()}</td>
											 <td data-t="${$(addspan3[z]).find('input').val()}">${$(addspan3[z]).text()}</td>
		                              		 <td ><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                              		 <td ><input class="num t-ran" name="return_ratio" type="text"></td>
									...
									// $('.table-div table tfoot tr td:nth-child(1)').attr('colspan','4')
									colspan();
								}
							} else {
								if ($($('.right-ul1')[1]).find('span').length != 0 && $($('.right-ul1')[2]).find('span').length != 0 && $($('.right-ul1')[3]).find('span').length != 0) {

									for (x = 0; x < addspan1.length; x++) {
										for (z = 0; z < addspan2.length; z++) {
											for (y = 0; y < addspan3.length; y++) {
												$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val() + ':' + $(addspan3[y]).find('input').val()}"></td>
											<td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}</td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                              		 <td ><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                              		 <td ><input class="num t-ran" name="return_ratio" type="text"></td>
		                                	 <td><input readonly class="num t-jie" name="supply_price" type="text"></td>
								...
								} else if ($($('.right-ul1')[1]).find('span').length == 0 && $($('.right-ul1')[2]).find('span').length != 0 && $($('.right-ul1')[3]).find('span').length != 0) {

									for (z = 0; z < addspan2.length; z++) {
										for (y = 0; y < addspan3.length; y++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan2[x]).find('input').val() + ':' + $(addspan3[z]).find('input').val()}"></td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                              		 <td ><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
									...
									for (x = 0; x < addspan1.length; x++) {
										for (y = 0; y < addspan3.length; y++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan3[z]).find('input').val()}"></td>
											<td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                              		 <td ><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
									...
									for (x = 0; x < addspan1.length; x++) {
										for (z = 0; z < addspan2.length; z++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${date[date.length - 1].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val()}"></td>
											<td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}</td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
		                              		 <td ><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
								...
								} else if ($($('.right-ul1')[3]).find('span').length != 0) {
									for (x = 0; x < addspan3.length; x++) {
										$('.table-div table tbody').append(`<tr><td data-t="${date[date.length - 1].id}">${txt}<input name="standard_id_str" type="hidden" value="${$(date[date.length - 1]).id + ':' + $(addspan3[x]).find('input').val()}"></td><td data-t="${$(addspan3[x]).find('input').val()}">${$(addspan3[x]).text()}</td>
												 <td><input class="num t-shi" name="market_price" type="text"></td>
												 <td><input class="num t-ben" name="detail_prices" type="text"></td>
												 <td ><input class="num t-ran" name="return_ratio" type="text"></td>
												 <td><input readonly class="num t-jie" name="supply_price" type="text"></td>
								...
								if ($($('.right-ul1')[1]).find('span').length != 0 && $($('.right-ul1')[2]).find('span').length != 0 && $($('.right-ul1')[3]).find('span').length != 0) {
									for (s = snu; s < date.length; s++) {
										for (x = 0; x < addspan1.length; x++) {
											for (z = 0; z < addspan2.length; z++) {
												for (y = 0; y < addspan3.length; y++) {
													$('.table-div table tbody').append(`<tr><td data-t="${date[s].id}">${date[s].spertAttrValue}<input name="standard_id_str" type="hidden" value="${date[s].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val() + ':' + $(addspan3[y]).find('input').val()}"></td>
												<td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}</td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                              		 <td><input class="num t-shi" name="market_price" type="text"></td>
									...
									for (s = snu; s < date.length; s++) {
										for (z = 0; z < addspan2.length; z++) {
											for (y = 0; y < addspan3.length; y++) {
												$('.table-div table tbody').append(`<tr><td data-t="${date[s].id}">${date[s].spertAttrValue}<input name="standard_id_str" type="hidden" value="${date[s].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val()}"></td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                              		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
									...
									for (s = snu; s < date.length; s++) {
										for (x = 0; x < addspan1.length; x++) {
											for (y = 0; y < addspan3.length; y++) {
												$('.table-div table tbody').append(`<tr><td data-t="${date[s].id}">${date[s].spertAttrValue}<input name="standard_id_str" type="hidden" value="${date[s].id + ':' + $(addspan1[x]).find('input').val() + ':' + $(addspan3[y]).find('input').val()}"></td>
												<td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                              		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
									...
									for (s = snu; s < date.length; s++) {
										for (x = 0; x < addspan2.length; x++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[s].id}">${date[s].spertAttrValue}<input name="standard_id_str" type="hidden" value="${date[s].id + ':' + $(addspan2[x]).find('input').val()}"></td><td data-t="${$(addspan2[x]).find('input').val()}">${$(addspan2[x]).text()}</td>
		                              		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                              		 <td><input class="num t-ran" name="return_ratio" type="text"></td>
		                                	 <td><input readonly class="num t-jie" name="supply_price" type="text"></td>
									...
									for (s = snu; s < date.length; s++) {
										for (x = 0; x < addspan3.length; x++) {
											$('.table-div table tbody').append(`<tr><td data-t="${date[s].id}">${date[s].spertAttrValue}<input name="standard_id_str" type="hidden" value="${date[s].id + ':' + $(addspan3[x]).find('input').val()}"></td><td data-t="${$(addspan3[x]).find('input').val()}">${$(addspan3[x]).text()}</td>
		                              		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                              		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                              		 <td><input class="num t-ran" name="return_ratio" type="text"></td>
		                                	 <td><input readonly class="num t-jie" name="supply_price" type="text"></td>
						...
						if ($($('.right-ul1')[1]).find('span').length == snu) {
							if ($($('.right-ul1')[1]).find('span').length == 1) {
								if (addspan2.length != 0 && addspan3.length != 0) {
									//疑问点
									//    for (x=0;x<addspan1.length;x++){
									// 	   for (z=0;z<addspan2.length;z++){
									// 		   for (y=0;y<addspan3.length;z++){
									// 		   $('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val()+':'+date[date.length-1].id+':'+$(addspan2[z]).find('input').val()}"></td><td data-t="${date[date.length-1].id}">${txt}</td>
									// 	 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
									// 	 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
									// 	 <td><input class="num t-shi" name="market_price" type="text"></td>
								...
								} else if ((addspan2.length == 0 && addspan3.length != 0) || (addspan2.length != 0 && addspan3.length == 0)) {
									var trt2 = $($('.right-ul1')[1]).children('li:nth-child(1)').text();
									$('.table-div table thead th:nth-child(1)').after(`<th data-t="${trt2}">${trt2}</th>`)
									$('.table-div table tbody tr td:nth-child(1)').after(`<td data-t="${date[date.length - 1].id}">${txt}</td>`)
									// $('.table-div table tfoot tr td:nth-child(1)').attr('colspan','3')
								...
								if (addspan2.length != 0 && addspan3.length != 0) {
									for (x = 0; x < addspan1.length; x++) {
										for (z = 0; z < addspan2.length; z++) {
											for (y = 0; y < addspan3.length; y++) {
												$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + date[date.length - 1].id + ':' + $(addspan2[z]).find('input').val() + ':' + $(addspan3[y]).find('input').val()}"></td><td data-t="${date[date.length - 1].id}">${txt}</td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                            		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                            		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
								...
								} else if (addspan2.length != 0 && addspan3.length == 0) {
									for (z = 0; z < addspan1.length; z++) {
										for (y = 0; y < addspan2.length; y++) {
											$('.table-div table tbody').append(`<tr>
											<td data-t="${$(addspan1[z]).find('input').val()}">${$(addspan1[z]).text()}
											<input name="standard_id_str" type="hidden" value="${$(addspan1[z]).find('input').val() + ':' + date[date.length - 1].id + ':' + $(addspan2[y]).find('input').val()}">
											</td>
											<td data-t="${date[date.length - 1].id}">${txt}</td>
											 <td data-t="${$(addspan2[y]).find('input').val()}">${$(addspan2[y]).text()}</td>
		                            		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                            		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                            		 <td><input class="num t-ran" name="return_ratio" type="text"></td>
		                              	 <td><input readonly class="num t-jie" name="supply_price" type="text"></td>
									...
									for (x = 0; x < addspan1.length; x++) {
										for (y = 0; y < addspan3.length; y++) {
											$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + date[date.length - 1].id + ':' + $(addspan3[y]).find('input').val()}"></td><td data-t="${date[date.length - 1].id}">${txt}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                            		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                            		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                            		 <td><input class="num t-ran" name="return_ratio" type="text"></td>
							...
							if (snu == 1) {
								...
								if ($($('.right-ul1')[2]).find('span').length == 0 && $($('.right-ul1')[3]).find('span').length == 0) {
									for (x = 0; x < addspan1.length; x++) {
										for (s = snu; s < date.length; s++) {
											$('.table-div table tbody').append(`<tr>
												<td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + date[s].id}"></td>
												<td data-t="${date[s].id}">${date[s].spertAttrValue}</td>
		                            		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                            		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                            		 <td><input class="num t-ran" name="return_ratio" type="text"></td>
								...
								} else if ($($('.right-ul1')[2]).find('span').length != 0 && $($('.right-ul1')[3]).find('span').length == 0) {
									for (x = 0; x < addspan1.length; x++) {
										for (s = snu; s < date.length; s++) {
											for (z = 0; z < addspan2.length; z++) {
												$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + date[s].id + ':' + $(addspan2[z]).find('input').val()}"></td><td data-t="${date[s].id}">${date[s].spertAttrValue}</td>
									...
									for (x = 0; x < addspan1.length; x++) {
										for (s = snu; s < date.length; s++) {
											for (z = 0; z < addspan3.length; z++) {
												$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + date[s].id + ':' + $(addspan3[z]).find('input').val()}"></td><td data-t="${date[s].id}">${date[s].spertAttrValue}</td>
											 <td data-t="${$(addspan3[z]).find('input').val()}">${$(addspan3[z]).text()}</td>
		                            		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                            		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                            		 <td><input class="num t-ran" name="return_ratio" type="text"></td>
									...
									for (x = 0; x < addspan1.length; x++) {
										for (s = snu; s < date.length; s++) {
											for (z = 0; z < addspan2.length; z++) {
												for (y = 0; y < addspan3.length; y++) {
													$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + date[s].id + ':' + $(addspan2[z]).find('input').val() + ':' + $(addspan3[y]).find('input').val()}"></td><td data-t="${date[s].id}">${date[s].spertAttrValue}</td>
											 <td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
											 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
		                            		 <td><input class="num t-shi" name="market_price" type="text"></td>
		                            		 <td><input class="num t-ben" name="detail_prices" type="text"></td>
							...
							if ($($('.right-ul1')[2]).find('span').length == 1) {
								...
								if (addspan3.length == 0) {
									...
									for (x = 0; x < addspan1.length; x++) {
										for (z = 0; z < addspan2.length; z++) {
											for (y = 0; y < addspan3.length; y++) {
												$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val() + ':' + date[date.length - 1].id}+ ':' + $(addspan3[y]).find('input').val()"></td>
										<td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
										 <td data-t="${date[date.length - 1].id}">${txt}</td>
										 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
	                            		 <td><input class="num t-shi" name="market_price" type="text"></td>
								...
								if (addspan3.length == 0) {
									...
									for (x = 0; x < addspan1.length; x++) {
										for (z = 0; z < addspan2.length; z++) {
											for (s = snu - 1; s < date.length; s++) {
												for (y = 0; y < addspan3.length; y++) {
													$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val() + ':' + date[s].id + ':' + $(addspan3[y]).find('input').val()}">
											</td><td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
												 <td data-t="${date[s].id}">${date[s].spertAttrValue}</td>
												 <td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
												 <td ><input class="num t-shi" name="market_price" type="text"></td>
												 <td><input class="num t-ben" name="detail_prices" type="text"></td>
						...
						var addspan1 = $($('.right-ul1')[0]).find('span');
						var addspan2 = $($('.right-ul1')[1]).find('span');
						var addspan3 = $($('.right-ul1')[2]).find('span');
						if ($($('.right-ul1')[3]).find('span').length == snu) {
							if ($($('.right-ul1')[3]).find('span').length == 1) {
								var trt2 = $($('.right-ul1')[3]).children('li:nth-child(1)').text();
								$('.table-div table thead th:nth-child(3)').after(`<th data-t="${trt2}">${trt2}</th>`)
								$('.table-div table tbody tr td:nth-child(3)').after(`<td data-t="${date[date.length - 1].id}">${txt}</td>`)
								// $('.table-div table tfoot tr td:nth-child(1)').attr('colspan','4');
								colspan();
								var nth1 = $('.table-div table tbody tr td:nth-child(1)');
								for (c = 0; c < nth1.length; c++) {
								...
								for (x = 0; x < addspan1.length; x++) {
									for (z = 0; z < addspan2.length; z++) {
										for (y = 0; y < addspan3.length; y++) {
											$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val() + ':' + $(addspan3[y]).find('input').val() + ':' + date[date.length - 1].id}"></td>
										<td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
										<td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
										 <td data-t="${date[date.length - 1].id}">${txt}</td>
	                            		 <td><input class="num t-shi" name="market_price" type="text"></td>
							...
							// }
								...
								for (x = 0; x < addspan1.length; x++) {
									for (z = 0; z < addspan2.length; z++) {
										for (y = 0; y < addspan3.length; y++) {
											for (s = snu - 1; s < date.length; s++) {
												$('.table-div table tbody').append(`<tr><td data-t="${$(addspan1[x]).find('input').val()}">${$(addspan1[x]).text()}<input name="standard_id_str" type="hidden" value="${$(addspan1[x]).find('input').val() + ':' + $(addspan2[z]).find('input').val() + ':' + $(addspan3[y]).find('input').val() + ':' + date[s].id}"></td>
												<td data-t="${$(addspan2[z]).find('input').val()}">${$(addspan2[z]).text()}</td>
												<td data-t="${$(addspan3[y]).find('input').val()}">${$(addspan3[y]).text()}</td>
												 <td data-t="${date[s].id}">${date[s].spertAttrValue}</td>
												 <td ><input class="num t-shi" name="market_price" type="text"></td>
				...
				$('.num').cbNum();
				$('.num1').cbNum();
				$('.t-shi').parent().css("display", "none");
				$('.t-ran').parent().css("display", "none");
				$('.t-jie').parent().css("display", "none");
				var tteText=commonNum;	
				funp('select'+tteText,tteText);
			}
		});
		//            if(txt!=undefined){
	...
	function addval(a, b) {
		if (a.siblings('input').val()) {
			var html = `<span>${a.siblings('input').val()}<b></b></span> <a href="javascript:;"><img src="${contextPath}/static/sbShop/images/add-a.png" alt=""></a>`;
...
function addtable() {
	...
	if (span3.length != 0) {
		if (span2.length == 0 && span1.length != 0) {
			htm += `<thead><th>${$('.standard-right ul:nth-child(2) li:first-child').text()}</th><th>${$('.standard-right ul:nth-child(4) li:first-child').text()}</th><th  style="display: none">市场价格（元）</th><th>本店价格（元）</th><th style="display: none">让利比例（%）</th><th style="display: none">结算价格（元）</th><th>会员价格（元）</th><th>库存数量</th></thead><tbody>`;
			for (i = 0; i < span1.length; i++) {
				for (n = 0; n < span3.length; n++) {
					htm += `<tr><td data-t="${$(span1[i]).find('input').val()}">${$(span1[i]).text()}<input type="hidden" name="standard_id_str" value="${$(span1[i]).children('input').val() + ':' + $(span3[n]).children('input').val()}"><input name="standard_value_str" type="hidden" value="${text1 + ':' + $(span1[i]).text() + ';' + text3 + ':' + $(span3[n]).text()}"></td><td data-t="${$(span3[n]).find('input').val()}">${$(span3[n]).text()}</td>
								<td  style="display: none"><input class="num t-shi" name="market_price"  type="text"></td>
                                <td><input class="num t-ben" name="detail_prices" type="text"></td>
                                <td  style="display: none"><input class="num t-ran" name="return_ratio" type="text"></td>
                                <td   style="display: none"><input readonly class="num t-jie" name="supply_price" type="text"></td>
                                <td ><input class="num t-huiy" name="member_price" type="text"></td>
                                <td><input class="num" name="detailInventory" type="text"></td>
                                </tr>`;
				}
			...
			htm += `<thead><th>${$('.standard-right ul:nth-child(2) li:first-child').text()}</th><th>${$('.standard-right ul:nth-child(3) li:first-child').text()}</th><th>${$('.standard-right ul:nth-child(4) li:first-child').text()}</th><th  style="display: none">市场价格（元）</th><th>本店价格（元）</th><th  style="display: none">让利比例（%）</th><th style="display: none">结算价格（元）</th><th>会员价格（元）</th><th>库存数量</th></thead><tbody>`;
			for (i = 0; i < span1.length; i++) {
				for (j = 0; j < span2.length; j++) {
					for (n = 0; n < span3.length; n++) {
						htm += `<tr><td data-t="${$(span1[i]).find('input').val()}">${$(span1[i]).text()}<input type="hidden" name="standard_id_str" value="${$(span1[i]).children('input').val() + ':' + $(span2[j]).children('input').val() + ':' + $(span3[n]).children('input').val()}"><input name="standard_value_str" type="hidden" value="${text1 + ':' + $(span1[i]).text() + ';' + text2 + ':' + $(span2[j]).text() + ';' + text3 + ':' + $(span3[n]).text()}"></td><td data-t="${$(span2[j]).find('input').val()}">${$(span2[j]).text()}</td><td data-t="${$(span3[n]).find('input').val()}">${$(span3[n]).text()}</td>
								<td  style="display: none"><input class="num t-shi" name="market_price"  type="text"></td>
                                <td><input class="num t-ben" name="detail_prices" type="text"></td>
                                <td style="display: none"><input class="num t-ran" name="return_ratio" type="text"></td>
                                <td   style="display: none"><input readonly class="num t-jie" name="supply_price" type="text"></td>
			...
			for (j = 0; j < span2.length; j++) {
				for (n = 0; n < span3.length; n++) {
					htm += `<tr><td data-t="${$(span2[j]).find('input').val()}">${$(span2[j]).text()}</td><td data-t="${$(span3[n]).find('input').val()}">${$(span3[n]).text()}</td>
								<td style="display: none"><input class="num t-shi" name="market_price"  type="text"></td>
                                <td><input class="num t-ben" name="detail_prices" type="text"></td>
                                <td  style="display: none"><input class="num t-ran" name="return_ratio" type="text"></td>
                                <td style="display: none"><input readonly class="num t-jie" name="supply_price" type="text"></td>
			...
			htm += `<thead><th>${$('.standard-right ul:nth-child(4) li:first-child').text()}</th><th style="display: none">市场价格（元）</th><th>本店价格（元）</th><th style="display: none">让利比例（%）</th><th style="display: none">结算价格（元）</th><th>会员价格（元）</th><th>库存数量</th></thead><tbody>`;

			for (n = 0; n < span3.length; n++) {
				htm += `<tr><td data-t="${$(span3[n]).find('input').val()}">${$(span3[n]).text()}</td>
								<td  style="display: none"><input class="num t-shi" name="market_price"  type="text"></td>
                                <td><input class="num t-ben" name="detail_prices" type="text"></td>
                                <td  style="display: none"><input  class="num t-ran" name="return_ratio" type="text"></td>
                                <td style="display: none"><input readonly class="num t-jie" name="supply_price" type="text"></td>
		...
		if (span2.length != 0) {
			htm += `<thead><th>${$('.standard-right ul:nth-child(2) li:first-child').text()}</th><th>${$('.standard-right ul:nth-child(3) li:first-child').text()}</th><th  style="display: none">市场价格（元）</th><th>本店价格（元）</th><th  style="display: none">让利比例（%）</th><th  style="display: none">结算价格（元）</th><th>会员价格（元）</th><th>库存数量</th></thead><tbody>`;
			for (i = 0; i < span1.length; i++) {
				for (j = 0; j < span2.length; j++) {
					htm += `<tr><td data-t="${$(span1[i]).find('input').val()}">${$(span1[i]).text()}<input name="standard_id_str" type="hidden" value="${$(span1[i]).children('input').val() + ':' + $(span2[j]).children('input').val()}"><input type="hidden" name="standard_value_str" value="${text1 + ':' + $(span1[i]).text() + ';' + text2 + ':' + $(span2[j]).text()}"></td><td data-t="${$(span2[j]).find('input').val()}">${$(span2[j]).text()}</td>
							<td style="display: none"><input class="num t-shi" name="market_price" type="text"></td>
		                    <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                    <td  style="display: none"><input class="num t-ran" name="return_ratio" type="text"></td>
		                    <td  style="display: none"><input readonly class="num t-jie" name="supply_price" type="text"></td>
			...
			htm += `<thead><th>${$('.standard-right ul:nth-child(2) li:first-child').text()}</th><th  style="display: none">市场价格（元）</th><th>本店价格（元）</th><th  style="display: none">让利比例（%）</th><th style="display: none">结算价格（元）</th><th>会员价格（元）</th><th>库存数量</th></thead><tbody>`;
			for (i = 0; i < span1.length; i++) {
				htm += `<tr><td data-t="${$(span1[i]).find('input').val()}">${$(span1[i]).text()}<input name="standard_id_str" type="hidden" value="${$(span1[i]).children('input').val()}"><input name="standard_value_str" type="hidden" value="${text1 + ':' + $(span1[i]).text()}"></td>
						<td  style="display: none"><input class="num t-shi" name="market_price" type="text"></td>
		                <td><input class="num t-ben" name="detail_prices" type="text"></td>
		                <td  style="display: none"><input class="num t-ran" name="return_ratio" type="text"></td>
		                <td   style="display: none"><input readonly class="num t-jie" name="supply_price" type="text"></td>
